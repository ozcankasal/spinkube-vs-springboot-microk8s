...
sudo microk8s kubectl label node --all spin=true --overwrite
...
echo "Ensuring Java 21 and C compiler are installed for builds..."
sudo apt-get update && sudo apt-get install -y openjdk-21-jdk-headless build-essential
